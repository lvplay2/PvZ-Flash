package com.popcap.flash.framework
{
   ...
   public class AppBase extends Sprite
   {
      ...
      private var mCheatBindings:Dictionary = new Dictionary();
      ...
      private var mPlugins:Vector.<BaseAppPlugin> = new Vector.<BaseAppPlugin>();
      ...
      private var mUpdateHooks:Array = new Array();
      
      private var _stateManager:CStateManager;
      
      private var mSaveDataSO:SharedObject = null;
      
      private var mServiceMap:Dictionary = new Dictionary(false);
      
      private var mDoStep:Boolean = false;
      
      private var _widgetManager:CWidgetManager;
      
      private var mCheats:Dictionary = new Dictionary(true);
      
      private var initialized:Boolean = false;
      
      private var mMusicManager:MusicManager;
      
      private var mReferences:Dictionary = new Dictionary(true);
      ...
      private function handleFrame(e:Event) : void
      {
         ...
         this.avgFlashTime += thisTime - this.lastFlashTime;
         this.excessUpdateTime += elapsed;
         ...
         this.avgUpdateTime += getTimer() - startTime;
         startTime = getTimer();
         this.renderStep();
         this.avgRenderTime += getTimer() - startTime;
         ++this.fpsCount;
      ...
      {
         var index:int = int(this.mUpdateHooks.indexOf(hook));
         if(index >= 0)
         {
      ...
      public function start() : void
      {
         ...
         var numBindings:int = int(this.mDataXML.cheats.bind.length());
      ...
      private function handleDataLoaded(e:Event) : void
      {
         ...
         this.mDataXML = xml;
         var len:int = int(xml.Plugins.Plugin.length());
      ...
      private function addBinding(binding:XML) : void
      {
         ...
         var code:int = int(this.mCodeMap[combo]);
         if(code > 0)
         {
         ...
         for each(piece in pieces)
         {
            ...
            {
               code = int(this.mCodeMap[matcher]);
               if(code == 0)
               {
                  return;
               }
