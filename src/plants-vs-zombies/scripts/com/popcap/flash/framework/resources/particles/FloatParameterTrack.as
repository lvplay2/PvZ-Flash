package com.popcap.flash.framework.resources.particles
{
   public class FloatParameterTrack
   {
      ...
      public function scale(scale:Number) : void
      {
         ...
         var aNumNodes:int = int(this.mNodes.length);
         for(var i:int = 0; i < aNumNodes; i++)
         {
            aNode = this.mNodes[i];
            aNode.mHighValue *= scale;
            aNode.mLowValue *= scale;
         }
      }
      ...
      public function evaluate(time:Number, interp:Number) : Number
      {
         ...
         var aNodeCount:int = int(this.mNodes.length);
         for(var i:int = 1; i < aNodeCount; i++)
         {
